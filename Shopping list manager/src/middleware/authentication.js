  ...
  return async (req, res, next) => {
    try {
      ...
      if (value === AUTH_MODE.getCurrentUser) {
        ...
        if (value === AUTH_MODE.hasAccess) shoppingListId = req.params.shoppingListId;
...
const trySwitch = async (value, decoded, ownerUserId, ownerShoppingListId, shoppingListId) => {
  switch (value) {
    ...
    case AUTH_MODE.hasAccess:
      // eslint-disable-next-line no-use-before-define
      return await checkHasAccess(decoded, shoppingListId);
    default:
 ...
 * @returns Boolean
 */
const checkHasAccess = async (decoded, shoppingListId) => {
  try {
    ...
    if (response) {
      ...
      const response = !!shoppingList.accessManagement.find((user) => user._id.toString() === decoded.id);
